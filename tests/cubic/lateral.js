/*
 * Le pas lateral du pion sur les panneaux lateraux, et l'e.p. qui va avec.
 *
 *   node tests/cubic/lateral.js
 *
 * CE QUI A CHANGE : cette suite n'AFFIRMAIT rien. Elle imprimait ce qu'elle
 * trouvait -- combien de cases offrent un pas lateral, si le coup se joue --
 * et sortait toujours avec le code 0. tests/run.js la comptait donc parmi les
 * suites qui passent quoi qu'il arrive : une regression sur le pas lateral
 * l'aurait traversee sans un mot. Les memes faits sont desormais VERIFIES ;
 * les affichages restent, ils disent ou regarder quand une verification tombe.
 */
...
t.check("les panneaux lateraux sont les faces 2 et 5", sides, [2,5]);
...
t.ok("des cases laterales offrent le pas de cote", anyLat > 0);
t.check("et aucune case de couloir ne l'offre", spurious, 0);
...
t.ok("le pas lateral est bien engendre comme coup jouable", !!latMove);
if(latMove){ b.ApplyMove(game,latMove); b.mWho=-b.mWho;
  ...
  // Un pion passe de cote doit continuer d'avancer : sans cela il serait
  // enferme sur le panneau, et le pas lateral deviendrait un piege.
  t.ok("et le pion avance encore apres etre passe de cote", !!fw);
}
...
t.check("vingt parties au hasard, aucune exception", crash, 0);
t.ok("et le pas lateral s'y joue reellement", lateralPlayed > 0);
t.done("Cubic : pas lateral");
