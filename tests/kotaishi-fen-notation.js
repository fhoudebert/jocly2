...
(async()=>{
  ...
  await m.load({game:'kotaishi-shogi', initialBoard:'6k6/13/13/13/6E6/13/13/13/6K6 w - - 0 1', playedMoves:[]});
  ...
  await m.load({game:'kotaishi-shogi', initialBoard:'6k6/13/13/13/6+e6/13/13/13/6K6 b - - 0 1', playedMoves:[]});
  ...
  m=await Jocly.createMatch('kotaishi-shogi');
  await m.load({game:'kotaishi-shogi', initialBoard:'6k6/13/13/13/6+E6/13/13/13/6K6 w - - 0 1', playedMoves:[]});
  d=dests(await strMoves(m),'e5').sort();
  console.log('    dest:',d.join(' '));
  ...
  m=await Jocly.createMatch('kotaishi-shogi');
  await m.load({game:'kotaishi-shogi', initialBoard:'6k6/13/13/13/6+e6/6E6/13/13/6K6 b - - 0 1', playedMoves:[]});
  let fen1=await m.getBoardState('fen');
  let m2=await Jocly.createMatch('kotaishi-shogi');
  await m2.load({game:'kotaishi-shogi', initialBoard:fen1, playedMoves:[]});
  let fen2=await m2.getBoardState('fen');
  console.log('    fen1:',fen1,'\n    fen2:',fen2);
  ok(fen1===fen2, 'export->import->export identique');

  // --- Item 2: le + de promotion apparaît dans la notation ---
  console.log('Test E - notation: la promotion éléphant->prince porte un + final');
  m=await Jocly.createMatch('kotaishi-shogi');
  await m.load({game:'kotaishi-shogi', initialBoard:'6k6/13/13/13/13/6e6/13/13/6K6 b - - 0 1', playedMoves:[]});
